...
import EmployeeProjectManager from './EmployeeProjectManager';
...
interface TimeTrackerProps {
  employees: Array<{ id: string; firstName: string; lastName: string; position: string; projectId?: string }>;
...
const TimeTracker: React.FC<TimeTrackerProps> = ({ 
  ...
  const handleSaveAll = () => {
    ...
    const entries = Object.entries(employeesData).filter(([employeeId, data]) => {
      ...
      <Card className="max-w-6xl mx-auto">
        ...
        <CardContent>
          <div className="space-y-6">
            ...
            {/* Zarządzanie pracownikami dla budowy */}
            {selectedProjectId && (
              <div className="p-4 bg-blue-50 rounded-lg">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-medium">Zarządzanie pracownikami</h3>
                  <Button
                    variant="outline"
                    onClick={() => setShowEmployeeManager(!showEmployeeManager)}
                  >
                    {showEmployeeManager ? 'Ukryj' : 'Pokaż'} zarządzanie pracownikami
                  </Button>
                </div>
